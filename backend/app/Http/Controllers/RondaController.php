        ...
        foreach ($rondas as $ronda) {
            foreach ($ronda->areas as $area) {
                ...
                foreach ($area->equipos as $equipo) {
                    ...
                    // Guardar historial de ubicaciones por placa
                    if ($equipo->placa) {
                        $clavePlaca = $equipo->placa;
                        if (!isset($ubicacionPorEquipo[$clavePlaca])) {
                            $ubicacionPorEquipo[$clavePlaca] = [
                                'nombre'    => $equipo->equipo_nombre,
                                'historial' => []
                            ];
                        }
                        $ubicacionPorEquipo[$clavePlaca]['historial'][] = [
                            'fecha'     => $ronda->fecha,
                    ...
                    if ($equipo->no_encontrado) {
                        $movimientos[] = [
                            'equipo'       => $equipo->equipo_nombre,
                            'placa'        => $equipo->placa,
                            'fecha'        => $ronda->fecha,
                            'areaAnterior' => $area->area_nombre,
                            'areaActual'   => 'No encontrado',
                            'tipo'         => 'perdido',
                        ];
                        continue;
                    }
                    ...
                    if ($equipo->placa) {
                        $key = $equipo->placa;
                        if (!isset($historialFallasPorEquipo[$key])) {
                            $historialFallasPorEquipo[$key] = [
                                'nombre'       => $equipo->equipo_nombre,
                                'placa'        => $equipo->placa,
                                'fechasFallas' => [],
                            ];
                        ...
                        if ($tieneFalla) {
                            if (!in_array($ronda->fecha, $historialFallasPorEquipo[$key]['fechasFallas'])) {
                                $historialFallasPorEquipo[$key]['fechasFallas'][] = $ronda->fecha;
                            }
                        }
                    }
                }
